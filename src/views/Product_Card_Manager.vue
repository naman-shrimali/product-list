<template>
  <div id="app" >
    ...
    <swiper-slide v-for="product_Detail in product_Details" :key="product_Detail.product_Id" ><ProductCard :product_Detail="product_Detail"  class="product_card" ></ProductCard> </swiper-slide>
  </swiper>
</div>
</template>
<script>
...
export default {
  ...
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vQlCd8xy1i8zZJKzS_6SkwM8NnSlQefBhIFbDKOwA_eUtMZ0_O53bMTdf8F9Oaepw-dHGjVy-x3-CsZ/pub?output=csv",
    ...
    console.log(this.product_Details.length)
    },
    methods: {
    fetchproduct_Details() {
      this.$papa.parse(this.product_Desc_Url, {
        download: true,
        header: true,
        complete: (results) => { this.product_Details = results.data;
              this.trial() }
    ...
    trial() {
      ...
      for(let j=0;j<this.product_Details.length;j++)
      {
        console.log(j);
        console.log(this.product_id);
        if( this.product_id===this.product_Details[j].product_Id)
        {
          this.helper = j;
        }
      }
      console.log(this.helper);
      this.swiper.slideTo(this.helper, 1000, false)
